...
from hummingbot.connector.exchange.farhadmarket.farhadmarket_order_book_tracker import FarhadmarketOrderBookTracker
from hummingbot.connector.exchange.farhadmarket.farhadmarket_user_stream_tracker import FarhadmarketUserStreamTracker
from hummingbot.connector.exchange.farhadmarket.farhadmarket_auth import FarhadmarketAuth
from hummingbot.connector.exchange.farhadmarket.farhadmarket_in_flight_order import FarhadmarketInFlightOrder
from hummingbot.connector.exchange.farhadmarket import farhadmarket_utils
from hummingbot.connector.exchange.farhadmarket import farhadmarket_constants as Constants
...
class FarhadmarketExchange(ExchangeBase):
    """
    FarhadmarketExchange connects with farhadmarket.com exchange and provides order book pricing, user account tracking
    ...
    def __init__(self,
                 farhadmarket_api_key: str,
                 farhadmarket_secret_key: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True
                 ):
        """
        :param farhadmarket_api_key: The API key to connect to private farhadmarket.com APIs.
        :param farhadmarket_secret_key: The API secret.
        ...
        self._farhadmarket_auth = FarhadmarketAuth(farhadmarket_api_key, farhadmarket_secret_key)
        self._order_book_tracker = FarhadmarketOrderBookTracker(trading_pairs=trading_pairs)
        self._user_stream_tracker = FarhadmarketUserStreamTracker(self._farhadmarket_auth, trading_pairs)
        self._ev_loop = asyncio.get_event_loop()
        self._shared_client = None
        self._poll_notifier = asyncio.Event()
        self._last_timestamp = 0
        self._in_flight_orders = {}  # Dict[client_order_id:str, FarhadmarketInFlightOrder]
    ...
    @property
    def in_flight_orders(self) -> Dict[str, FarhadmarketInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        ...
        self._in_flight_orders.update({
            key: FarhadmarketInFlightOrder.from_json(value)
    ...
    async def check_network(self) -> NetworkStatus:
        ...
        try:
            ...
            await self._api_request("LAST", "markets/BTC_USDT")
    ...
    def buy(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        """
        order_id: str = farhadmarket_utils.get_new_client_order_id(True, trading_pair)
    ...
    def sell(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        :returns A new internal order id
        """
        order_id: str = farhadmarket_utils.get_new_client_order_id(False, trading_pair)
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = FarhadmarketInFlightOrder(
    ...
    async def _update_balances(self):
        ...
        account_info = await self._api_request("OVERVIEW", "balances", {}, True)
        for account in account_info:
            asset_name = account["name"]
            self._account_available_balances[asset_name] = Decimal(str(account["available"]))
            self._account_balances[asset_name] = Decimal(str(account["freeze"])) + Decimal(str(account["available"]))
    ...
    def _process_order_message(self, order_msg: Dict[str, Any]):
        ...
        if tracked_order.is_cancelled:
            ...
            self.logger().info(f"The market order {client_order_id} has failed according to order status API. "
                               f"Reason: {farhadmarket_utils.get_api_reason(order_msg['reason'])}")
            self.trigger_event(MarketEvent.OrderFailure,
                               MarketOrderFailureEvent(
                                   self.current_timestamp,
                                   client_order_id,
